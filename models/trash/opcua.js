 ...
 * OPC UA Model
 * 
 * Scope issue: JS only sees direct function calls in the class opcuaPro as a method of this class.
 * Whenever a function is passed as a callback to a method, the scope of the passed function is
 * moved to the method. In the method, the function cannot longer see the "this." scope. Therefore,
 * a 'global' object (in this case var opcua) is needed. The opcua-object is mostly used in callback
 * methods (cbMethodName), but can be necessary in other functions as well.
 * 
 * @author Thomas Frei
 * @date 2014-10-16
 */
  ...
  opcuaPro.prototype = {
    ...
    cbConnected : function(err) {
      if (!err) {
        ...
        opcua.createSession(); // the cb is called in this.client.connect() and in this scope, the
                                // this.-object does not see the opcuaPro methods. Therefore a
